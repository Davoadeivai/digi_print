...
export default function ReviewForm({ productId, onSuccess }: Props) {
  ...
  const submitReview = async () => {
    if (rating === 0) { toast.error('لطفا امتیاز را انتخاب کنید'); return; }
    if (!review.trim()) { toast.error('لطفا متن نظر را وارد کنید'); return; }
    try {
      setLoading(true);
    ...
    <Card className="shadow-lg">
      ...
      <CardContent className="space-y-4">
        ...
        <Button onClick={submitReview} disabled={loading} className="w-full bg-purple-600 hover:bg-purple-700">
          {loading ? 'در حال ارسال...' : <>ارسال نظر <Send className="w-4 h-4 mr-2 inline" /></>}
        </Button>
      </CardContent>
    </Card>
  );
